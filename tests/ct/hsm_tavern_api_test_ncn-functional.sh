...
{
    echo "cleaning up..."
    if [[ -d ${HSM_TEST_DIR}/.pytest_cache ]] ; then
        rm -rf ${HSM_TEST_DIR}/.pytest_cache
    fi
...
if [[ ! -d ${HSM_TEST_DIR} ]] ; then
    >&2 echo "ERROR: failed to locate Tavern test directory: ${HSM_TEST_DIR}"
    cleanup
    exit 1
else
    TEST_DIR_FILES=$(ls ${HSM_TEST_DIR})
    TEST_DIR_TAVERN_FILES=$(echo "${TEST_DIR_FILES}" | grep -E "^test_.*\.tavern\.yaml")
    if [[ -z "${TEST_DIR_TAVERN_FILES}" ]] ; then
        >&2 echo "ERROR: no Tavern tests in CT test directory: ${HSM_TEST_DIR}"
